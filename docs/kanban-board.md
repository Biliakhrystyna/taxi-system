...
### In Progress / Backlog
- [ ] Реальна безпечна маршрутизація через OpenRouteService (`alternative_routes`) замість
      синусоїди в `routeBuilder.ts` — обирати альтернативу з найменшою кількістю поворотів
      (аналіз зміни азимута між сегментами геометрії), а не найкоротшу/найшвидшу.
      Ведеться в окремій гілці через ризикованість (зовнішній API, зміна серверної й
      клієнтської логіки маршруту одночасно).
